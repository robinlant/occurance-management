...
A self-hosted duty and occurrence management platform for teams. Members sign up for recurring duties, organizers schedule and manage slots, and the leaderboard tracks fair workload distribution.

**Features:** occurrence scheduling with participant limits · self-service sign-up and withdrawal · calendar and list views · leaderboard with date-range filtering · groups to categorize duties · out-of-office periods with conflict detection · role-based access (admin / organizer / participant) · profile pages with activity heatmap · automated email digests · English and German UI
...
```bash
# Current latest is 006_occurrence_created_at.sql, so:
...
|------|-----------------|
| **admin** | Everything. Manage users, reset passwords, manage groups, create/edit/delete occurrences, assign and remove participants. |
| **organizer** | Create, edit, and delete occurrences. Assign and remove participants. Manage groups. Cannot manage users. |
| **participant** | View occurrences and calendar. Sign up for and withdraw from open slots. Manage own profile and out-of-office periods. |
